...
和任务列表界面。TXT、Markdown、PDF、PNG 和 JPEG 已接入真实
`llamask-core` 后台扫描；敏感扫描草稿只保存在 Rust 内存。图片/PDF 复核只
按需取得受限尺寸的重编码页面预览和几何框；文本复核只取得命中内容及前后
各 80 个 Unicode 字符，不返回源路径或整份文档。用户可以编辑文本替换值、
接受或保留结果、移动/缩放遮罩、新增/删除手动遮罩，并通过原生保存对话框
调用安全导出与独立残留复扫。下一增量接入剪贴板和 Office，随后补批量输出。
开发和安全边界见桌面端架构文档。
