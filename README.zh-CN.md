...
和任务列表界面。PDF、PNG 和 JPEG 已接入真实 `llamask-core` 后台扫描，包含
运行文件完整性校验、无路径/无原文进度事件和合作式取消；敏感扫描草稿只
保存在 Rust 内存。复核页只按需取得受限尺寸的重编码页面预览和几何框，支持
接受或保留检测结果、移动/缩放遮罩、新增/删除手动遮罩，并通过原生保存对话
框调用现有安全导出与独立残留复扫；OCR 原文和源路径不会返回 WebView。
下一增量把文本、Office、剪贴板和批量输出接到相同状态机。开发和安全边界
见桌面端架构文档。
