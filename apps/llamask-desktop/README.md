...
格式/大小预检，并接入剪贴板、TXT、Markdown、DOCX、XLSX、PPTX、PDF、PNG、JPEG 的真实后台
...
- PPTX 文字复核只显示幻灯片、备注、批注、母版、版式或关系图及安全序号，
  不发送包内路径或整个演示文稿；母版与版式命中会提示可能影响多张页面。
- PPTX 内嵌 PNG/JPEG 复用源哈希校验、重编码预览和全局导出门禁。图表、
  外部数据、嵌入对象、宏、ActiveX 和不支持媒体在扫描阶段阻断。
...
- 批量导出只选择一次原生目录，顺序处理已自动确认或人工复核的文件；待复核
