# 桌面端 MVP 架构与 XLSX 里程碑

文档状态：V0.7，XLSX 扫描、复核与安全导出实现基线（2026-08-03）
...
`llamask-core` 文本、图片、PDF、DOCX 和 XLSX 扫描，并继续沿用同一条可信路径边界。
...
PDF、PNG 和 JPEG。Rust 后台工作线程按顺序处理文件，避免并发启动大量模型
或 OCR 进程；扫描任务草稿始终保存在 Rust 进程内。文本和不含内嵌图片的
DOCX/XLSX 在 OCR 未安装时仍可使用规则扫描，有完整运行注册表时叠加可选
...
标签；XLSX 返回工作表序号、单元格坐标和内容类型，但都不返回 OOXML
locator。响应不含绝对路径或整份文档。用户可以为每项结果编辑最多 256 个
字符的替换值，并明确选择
...
中调用既有 `llamask-core` 文本/图片/PDF/DOCX/XLSX 导出器，拒绝覆盖源文件
...
3. 支持 TXT/Markdown、DOCX、XLSX、PDF 和 PNG/JPEG，生成统一状态摘要。
...
丢弃结果；后续持久 sidecar 会加入请求级中断。下一增量将把 PPTX 接到相同
任务状态机。
...
- 剪贴板文本复用相同复核界面；最终按钮显示“复制脱敏结果”，不打开文件
  保存对话框。
- PPTX 复核将以内容域、类型和上下文分组，不渲染整个可执行文档。
...
- TXT/Markdown、DOCX、XLSX、PDF/图片真实扫描可以后台运行、报告进度并取消。
