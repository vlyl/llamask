# 桌面端 MVP 架构与 DOCX 里程碑

文档状态：V0.6，DOCX 扫描、复核与安全导出实现基线（2026-08-03）
...
`llamask-core` 文本、图片、PDF 和 DOCX 扫描，并继续沿用同一条可信路径边界。
...
一次启动当前会话内全部可扫描剪贴板文本、TXT、Markdown、DOCX、PDF、PNG
和 JPEG。Rust 后台工作线程按顺序处理文件，避免并发启动大量模型或 OCR
进程；扫描任务草稿始终保存在 Rust 进程内。文本和不含内嵌图片的 DOCX 在
OCR 未安装时仍可使用规则扫描，有完整运行注册表时叠加可选本地文本模型。
含 PNG/JPEG 的 DOCX 必须使用 OCR 建立图片子任务；缺少 OCR、不支持媒体、
嵌入对象或主动内容会安全阻断。取消采用合作式语义：PDF 在页边界终止并由
临时目录自动清理，文本/Office/图片在当前检测调用返回后丢弃结果，不生成
伪成功任务。
...
页面、图片或 DOCX 内嵌图片最长边不超过 2400 像素的重编码 PNG、原始坐标
系尺寸和遮罩几何信息。DOCX 图片还会验证图片任务与包内 entry 的一一对应。
响应不含绝对路径、OCR 行文本或命中原值；前端不持久化预览。

### `review_text_scan(id)` / `set_text_review_finding(...)`

文本和 DOCX 文字复核仅返回任务 id、总字符数，以及每个命中的类型、置信度、
命中内容、替换值和前后各 80 个 Unicode 字符。DOCX 额外返回按内容域编号的
安全标签，例如“页眉 · 内容 1”，但不返回 OOXML locator。响应不含绝对路径
或整份文档。用户可以为每项结果编辑最多 256 个字符的替换值，并明确选择
...
中调用既有 `llamask-core` 文本/图片/PDF/DOCX 导出器，拒绝覆盖源文件和已有
输出。DOCX 沿用最小 OOXML 修改、媒体去元数据重编码和包级结构清理。候选
副本必须通过适用的规则、可选模型、OCR、目标值和文件结构复扫后才原子
落盘；普通状态事件只返回输出文件名、完成度和稳定错误码。
...
3. 支持 TXT/Markdown、DOCX、PDF 和 PNG/JPEG，生成统一状态摘要。
...
丢弃结果；后续持久 sidecar 会加入请求级中断。扫描编排器稳定后，将依次把
XLSX、PPTX 接到相同任务状态机。
...
- 剪贴板文本复用相同复核界面；最终按钮显示“复制脱敏结果”，不打开文件
  保存对话框。
- XLSX/PPTX 复核将以内容域、类型和上下文分组，不渲染整个可执行文档。
...
- TXT/Markdown、DOCX、PDF/图片真实扫描可以后台运行、报告进度并取消。
